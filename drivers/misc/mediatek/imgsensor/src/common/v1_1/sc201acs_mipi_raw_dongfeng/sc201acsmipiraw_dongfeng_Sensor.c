...
#include "sc201acsmipiraw_dongfeng_Sensor.h"
#include "imgsensor_common.h"


/****************************Modify Following Strings for Debug****************************/
#define PFX "sc201acs_mipi_dongfeng"

#define LOG_1 LOG_INF("SC201ACS,MIPI 1LANE\n")
...
#define DEVICE_VERSION_DONGFENG_SC201ACS    "sc201acs"
static kal_uint8 deviceInfo_register_value = 0x00;

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SC202CS_SENSOR_ID_RADO,
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO,    //ADD,2021/8/26,fix depth sc201acs output_dataformat
...
//#ifdef __GAINMAP__
#if 1
#define SC201ACS_SENSOR_GAIN_BASE             1024
#define SC201ACS_SENSOR_GAIN_MAX              16384 //(16 * SC201ACS_SENSOR_GAIN_BASE)
#define SC201ACS_SENSOR_GAIN_MAX_VALID_INDEX  6
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    kal_uint16 reg_gain = gain << 4;

    if (reg_gain < SC201ACS_SENSOR_GAIN_BASE) {
        reg_gain = SC201ACS_SENSOR_GAIN_BASE;
    } else if (reg_gain > SC201ACS_SENSOR_GAIN_MAX) {
        reg_gain = SC201ACS_SENSOR_GAIN_MAX;
    }
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC201ACS_AGC_Param[SC201ACS_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC201ACS_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC201ACS_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC201ACS_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC201ACS_SENSOR_GAIN_BASE / SC201ACS_AGC_Param[gain_index][0];
        ...
        do {
			*sensor_id = return_sensor_id();
            pr_err("cwd return_sensor_id:0x%x\n",*sensor_id);
             if (*sensor_id == imgsensor_info.sensor_id - 0x4) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                imgsensor_info.module_id = 0x00;
             if (deviceInfo_register_value == 0x00) {
                    register_imgsensor_deviceinfo("Cam_r1", DEVICE_VERSION_DONGFENG_SC201ACS, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id - 0x4 != sensor_id)
       return ERROR_SENSOR_CONNECT_FAIL;
