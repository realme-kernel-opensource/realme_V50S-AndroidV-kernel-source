...
** File: oplus24251_txd_ili9883c_hd_dsi_vdo.c
...
#include "../mediatek/mediatek_v2/mtk_corner_pattern/oplus24251_txd_ili9883c_data_hw_roundedpattern.h"
/* #endif */
#include "include/oplus24251_txd_ili9883c_hd_dsi_vdo.h"
#include "../bias/oplus23661_aw37501_bias.h"
#include <linux/reboot.h>

#define MAX_NORMAL_BRIGHTNESS    (3510)

static int cabc_status = 0;
static int esd_brightness;
static unsigned char g_gammaflag = 0;
static bool lcm_first_boot = false;
...
static struct mtk_panel_params ext_params_60HZ = {/* 60hz */
	.pll_clk = MIPI_CLK,
	.data_rate = DATA_RATE,
	.vendor = "oplus24251_al5",
	.manufacture = "txd_ili9883c",
	.cust_esd_check = 0,
	.esd_check_enable = 1,
	...
	void *handle, unsigned int level)
{
	...
	char cabc_cmd_1[] = {0x55, 0x01};
	char dimming_set_on[] = {0x53, 0x2C};
	char dimming_set_off[] = {0x53, 0x24};
	char all_pixel_off[] = {0x22, 0x00};
	...
	if (level == 0) {
		if (g_shutdown_flag == 1) {
			pr_info("%s: backlight =%d, g_shutdown_flag=%d\n", __func__, level, g_shutdown_flag);
			cb(dsi, handle, dimming_set_on, ARRAY_SIZE(dimming_set_on));
			cb(dsi, handle, all_pixel_off, ARRAY_SIZE(all_pixel_off));
			usleep_range(6000, 6100);
			dimming_on = true;
		} else {
			cb(dsi, handle, dimming_set_off, ARRAY_SIZE(dimming_set_off));
			dimming_on = false;
		}
	}
	...
	if (dimming_on  == false && oplus_display_brightness > 0) {
		cb(dsi, handle, dimming_set_on, ARRAY_SIZE(dimming_set_on));
		dimming_on = true;
	}

	if (lcm_first_boot == true && cabc_true_mode == 1) {
		cb(dsi, handle, dimming_set_on, ARRAY_SIZE(dimming_set_on));
		cb(dsi, handle, cabc_cmd_1, ARRAY_SIZE(cabc_cmd_1));
		lcm_first_boot = false;
	}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	pr_err("[TP]  oplus24251_txd_ili9883c_dsi_vdo_lcm_drv g_shutdown_flag is %d, esd_flag is %d\n", g_shutdown_flag, esd_flag);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	register_device_proc("lcd", "oplus24251_al5", "txd_ili9883c");
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "oplus24251_txd_ili9883c_hd_dsi_vdo",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("oplus24251 txd ili9883c Panel Driver");
